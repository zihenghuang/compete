...
#' @export
I_SI_20136<-function(Mk,p=c(1,0,0,0),a_max=50,nTries=30){
    original=Mk
    Mk=transform(Mk)
    attempt=1
    n<-ncol(Mk)
                ...
                if (p1==0){
                    for (i in 1:(n-1)){
                        for (j in (i+1):n){
                            if (matrix1[i,j]<0){
                                if (j-i==1) {sum1=1}
                                if (j-i>1)  {sum1=1-sum(matrix1[i,(i+1):(j-1)])+sum(matrix1[j,(i+1):(j-1)])}
                                if (sum1>0){
                    ...
                    for (i in 1:n){
                        ...
                        for (j in 1:n){
                            if (i==j){
                                ...
                                record[j]=sum(sign(floor(matrix1[i,(i+1):j])))
                            }
                            else if(j<i){
                                record[j]=-sum(sign(floor(matrix1[i,j:(i-1)])))
                            }
                        }
                        if (min(record)<0){
                    ...
                    for (i in 1:n){
                        ...
                        for (j in 1:n){
                            if (i==j){
                                ...
                                record[1,j]=sum(sign(floor(matrix1[i,(i+1):j])))
                            }
                            else if(j<i){
                                record[1,j]=-sum(sign(floor(matrix1[i,j:(i-1)])))
                            }
                        }
                        if ((min(i,j)==1)|(max(i,j)==n)){
                            record[2,j]=fun(shift(matrix1,i,j))[2]-lll[2]
                        }
                        else{
