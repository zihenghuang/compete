...
#' @export
I_IS_2013<-function(Mk,p=0.5,a_max,nTries){
    original=Mk
    Mk=transform(Mk)
    attempt=1
    n<-ncol(Mk)
...
#                else if (random_p < sum(p[1:3]) & random_p >= sum(p[1:2])){
                    for (i in 1:n){
                        record=rep(0,n)
                        for (j in 1:n){
                            if (i==j){
                                ...
                                record[j]=sum(sign(floor(M[i,(i+1):j])))
                            }
                            else if(j<i){
                                record[j]=-sum(sign(floor(M[i,j:(i-1)])))
                            }
                        }
                        if (min(record)<0){
                            min1<-which(record==min(record))
                            rand<-sample(length(min1),1)
                            min1=min1[rand]
                            M<-shift(M,i,min1)
                    ...
                    for (i in 1:n){
                        ...
                        for (j in 1:n){
                            if (i==j){
                                ...
                                record[1,j]=sum(sign(floor(M[i,(i+1):j])))
                            }
                            else if(j<i){
                                record[1,j]=-sum(sign(floor(M[i,j:(i-1)])))
                            }
                        }
                        if ((min(i,j)==1)|(max(i,j)==n)){
                            record[2,j]=fun(shift(M,i,j))[2]-lll[2]
                        }
                        else{
                            record[2,j]=delta_si(M,i,j)
                        }
                        a1=record[1,];a2=record[2,]
                        if (min(a1)==0){
                            a2[-which(a1==0)]=0
                            if (min(a2)<0){
                                rand=sample(length(which(a2==min(a2))),1)
                                final=which(a2==min(a2))[rand]
                                M<-shift(M,i,final)
...
delta_si=function(M,i,j){
    ...
    if (i==j){
        ...
        u=sum(floor((Matrix[1:(i-1),(i+1):j]-1)/-2))
        w=sum(floor((Matrix[(i+1):j,(j+1):n]-1)/-2))
        x=sum(floor((Matrix[1:(i-1),i]-1)/-2))
