...
I_SI_1998<-function(Mk,nTries){
    ...
    M=matrix_change(Mk,index); matrix1 = (M-t(M))/2
    ...
    for (k in 1:length(best)){
        correlation[k]=cor.test(nature(Mk),best[[k]])$p.value
    }
        num<-which(correlation==min(correlation))
        result_1=list()
        result_2=list()
        for (i in 1:length(num)){
            result_1[[i]]=matrix_change(Mk,best[[num[i]]])
