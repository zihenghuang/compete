...
#' @export
I_IS_2013<-function(Mk,p=c(.3,.5,.1,.1),a_max=30,nTries=20){
    original=Mk
    Mk=transform(Mk)
    attempt=1
    n<-ncol(Mk)
            ...
            while (stopIteration2==FALSE){
                ...
                #####strategy4
                    ...
                    for (i in 1:n){
                        mini=0;minsi=0
                        for (j in 1:n){
                        delta=fun(shift(matrix1,i,j))-lll 
                        if (delta[1]<mini) {exchange=j;mini=delta[1];minsi=delta[2]}
                        if ((delta[1]==mini)&(delta[2]<minsi)) {exchange=j;minsi=delta[2]}                         
                        }
                        if ((mini<0)|(minsi<0)){
                            matrix1<-shift(matrix1,i,exchange)
                            index<-shift_index(index,i,exchange)
                            stopIteration2=FALSE;lll=fun(matrix1)
                        }
                    }
                                }
   #####strategy5
                    ...
                    for (i in 1:n){
                        for (j in 1:n){
                            delta=fun(shift(matrix1,i,j))-lll
                            if ((delta[1]<0)|((delta[1]==0)&(delta[2]<0))){
