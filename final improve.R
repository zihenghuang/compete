            ...
            while ((stopIteration2==FALSE)&(count_1<7)){
              ...
              count_1=count_1+1
                    for (i in 1:n){
                        ...
                        if (min(record)<0){
                            rand<-sample(which(record==min(record)),1)
                ...
                if ((stopIteration2==TRUE)&(count_2<2)){
                        lll=fun(matrix1)
                        for (i in 1:n){
                            for (j in 1:n){
                                delta_i=0
                                ...
                                if((fun(shift(matrix1,i,j))[2]-lll[2]<0)&(delta_i==0)&(i!=j)){
                                    matrix1<-shift(matrix1,i,j)
                                    index<-shift_index(index,i,j)
                                    lll=fun(matrix1)
                                   count_2=count_2+1
...
I_SI_20136(data,p=c(0,0,0,1),a_max=5,nTries=5)
